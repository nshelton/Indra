...
FloatParameter::FloatParameter(const char *name, float min, float max, float defaultVal)
    : m_value(defaultVal), m_default(defaultVal), m_min(min), m_max(max)
{
    m_name = name;
}
...
{
    int location = shader->getUniformLocationCached(m_name.c_str());
    if (location >= 0)
    {
...
Vec3Parameter::Vec3Parameter(const char *name, float min, float max, const vec3 &defaultVal)
    : m_value(defaultVal), m_default(defaultVal), m_min(min), m_max(max)
{
    m_name = name;
}
...
ColorParameter::ColorParameter(const char *name, const color &defaultVal)
    : m_value(defaultVal), m_default(defaultVal)
{
    m_name = name;
}
...
IntParameter::IntParameter(const char *name, int min, int max, int defaultVal)
    : m_value(defaultVal), m_default(defaultVal), m_min(min), m_max(max)
{
    m_name = name;
}
